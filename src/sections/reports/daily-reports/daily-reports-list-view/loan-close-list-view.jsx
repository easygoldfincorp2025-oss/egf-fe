...
export default function LoanCloseListView({ closedLoans }) {
  ...
  const chargeCashAmt = dataFiltered.reduce(
    (prev, next) => prev + (Number(next?.chargePaymentDetail?.chargeCashAmount) || 0),
    0
  );
  const chargeBankAmt = dataFiltered.reduce(
    (prev, next) => prev + (Number(next?.chargePaymentDetail?.chargeBankAmount) || 0),
    0
  );
            ...
            <Table size={table.dense ? 'small' : 'medium'} sx={{ minWidth: 'auto' }}>
              ...
              <TableBody>
                ...
                {dataFiltered.length == 0 && (
                  <TableRow>
                    <TableCell colSpan={18} align="center" sx={{ p: 1, fontWeight: 500 }}>
                      No Data Available
                    </TableCell>
                  </TableRow>
                )}
                ...
                >
                  ...
                  <TableCell sx={{ fontWeight: '600', color: '#637381', py: 1, px: 2 }}></TableCell>{' '}
                  <TableCell sx={{ fontWeight: '600', color: '#637381', py: 1, px: 2 }}>
                    {chargeCashAmt.toFixed(0)}
                  </TableCell>{' '}
                  <TableCell sx={{ fontWeight: '600', color: '#637381', py: 1, px: 2 }}>
                    {chargeBankAmt.toFixed(0)}
                  </TableCell>{' '}
                  <TableCell sx={{ fontWeight: '600', color: '#637381', py: 1, px: 2 }}></TableCell>
                  <TableCell sx={{ fontWeight: '600', color: '#637381', py: 1, px: 2 }}>
                    {netAmt.toFixed(0)}
