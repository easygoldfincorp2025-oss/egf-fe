...
export default function CaratPdf({ configs, carat }) {
  ...
  const remainingRows = carat.length - rowsPerPageFirst;
  ...
  const renderRow = (row, index, isLastRow) => {
    ...
    return (
      ...
      >
        ...
        <Text style={[styles.tableCell, { flex: 1 }]}>{row?.name || '-'}</Text>
  ...
  if (carat.length > rowsPerPageFirst) {
    for (let pageIndex = 0; pageIndex < additionalPages; pageIndex++) {
      const startIndex = rowsPerPageFirst + pageIndex * rowsPerPageOther;
      const endIndex = Math.min(startIndex + rowsPerPageOther, carat.length);
      const isLastPage = endIndex === carat.length;

      const pageRows = carat.slice(startIndex, endIndex).map((row, index) => {
        const actualIndex = startIndex + index;
        return renderRow(row, actualIndex, actualIndex === carat.length - 1);
      });
