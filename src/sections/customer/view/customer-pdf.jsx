...
export default function CustomerPdf({ configs, customer, filterData }) {
  ...
  const headers = [
    ...
    { label: 'Name', flex: 3.2 },
    { label: 'Branch', flex: 1 },
    { label: 'Aadhar Verified', flex: 0.6 },
    { label: 'Contact', flex: 0.8 },
    { label: 'Code', flex: 0.3 },
    { label: 'Joining Date', flex: 0.7 },
    { label: 'Status', flex: 0.4 },
    { label: 'IsLoan', flex: 0.5 },
  ];
  ...
  const remainingRows = customer.length - rowsPerPageFirst;
    ...
    return (
      ...
      >
        ...
        <Text style={[styles.tableCell, { flex: 0.6 }]}>
          {row.isAadharVerified ? 'Verified' : 'Unverified' || '-'}
        </Text>
        <Text style={[styles.tableCell, { flex: 0.8 }]}>{row.contact || '-'}</Text>
        <Text style={[styles.tableCell, { flex: 0.3 }]}>{row.customerCode || '-'}</Text>
        <Text style={[styles.tableCell, { flex: 0.7 }]}>{fDate(row.joiningDate) || '-'}</Text>
        <Text style={[styles.tableCell, { flex: 0.4, }]}>{row.status || '-'}</Text>
        <Text style={[styles.tableCell, { flex: 0.5, borderRight: 0 }]}>{row.isLoan === true ? "Active" : "inActive" || '-'}</Text>
      </View>
    );
  };
  ...
  if (customer.length > rowsPerPageFirst) {
    for (let pageIndex = 0; pageIndex < additionalPages; pageIndex++) {
      const startIndex = rowsPerPageFirst + pageIndex * rowsPerPageOther;
      const endIndex = Math.min(startIndex + rowsPerPageOther, customer.length);
      const isLastPage = endIndex === customer.length;

      const pageRows = customer.slice(startIndex, endIndex).map((row, index) => {
        const actualIndex = startIndex + index;
        return renderRow(row, actualIndex, actualIndex === customer.length - 1);
      });
