...
import { useGetAllUnsecurePartPayment } from '../../../api/unsecure-part-payment.js';
...
function LoanPartPaymentForm({ currentLoan, mutate }) {
  ...
  const { partPayment, refetchPartPayment } = useGetAllUnsecurePartPayment(currentLoan._id);
        ...
        <TableBody>
          {partPayment.map((row, index) => (
            <TableRow key={index} hover>
              ...
              <TableCell sx={{ whiteSpace: 'nowrap', py: 0, px: 2 }}>{row.amountPaid.toFixed(2)}</TableCell>
              <TableCell
                sx={{
                  whiteSpace: 'nowrap',
                  py: 0,
