...
export default function UnsecureLoanpayhistoryTableRow({ row, selected, onDeleteRow, loanStatus }) {
  const {
    loanNo,
    customer,
    scheme,
      ...
      <TableRow hover selected={selected} sx={{ backgroundColor: statusColors[status] || '' }}>
        ...
        <TableCell sx={{ whiteSpace: 'nowrap' }}>
          {getResponsibilityValue('update_loan_pay_history', configs, user) ? (
            <Link
              to={paths.dashboard.unsecureloanPayHistory.edit(_id)}
        ...
        <TableCell sx={{ whiteSpace: 'nowrap' }}>{loanAmount.toFixed(2)}</TableCell>
        <TableCell sx={{ whiteSpace: 'nowrap' }}>{interestLoanAmount.toFixed(2)}</TableCell>
        <TableCell sx={{ whiteSpace: 'nowrap' }}>{loanAmount - interestLoanAmount}</TableCell>
        <TableCell sx={{ whiteSpace: 'nowrap' }}>
          {scheme.interestRate < 1 ? scheme.interestRate : (scheme.interestRate - 1).toFixed(2)}
        </TableCell>
        <TableCell sx={{ whiteSpace: 'nowrap' }}>{cashAmount.toFixed(2)}</TableCell>
        <TableCell sx={{ whiteSpace: 'nowrap' }}>
          <Label
            variant="soft"
            color={
...
UnsecureLoanpayhistoryTableRow.propTypes = {
