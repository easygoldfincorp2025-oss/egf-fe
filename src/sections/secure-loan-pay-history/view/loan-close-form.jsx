...
import { useGetSecureCloseLoan } from '../../../api/secure-loan-close.js';
...
function LoanCloseForm({ currentLoan, mutate }) {
  ...
  const { loanClose, refetchLoanClose } = useGetSecureCloseLoan(currentLoan._id);
  ...
  const sendPdfToWhatsApp = async (item, date, charge) => {
    try {
      const blob = await pdf(
        <Noc nocData={currentLoan} closingDate={date} charge={charge} configs={configs} />
      ).toBlob();
        ...
        <TableBody>
          {loanClose.map((row, index) => (
            <TableRow key={index}>
              ...
              <TableCell sx={{ whiteSpace: 'nowrap', py: 0.5, px: 2 }}>
                {row.loan.loanAmount- row.closingCharge}
              </TableCell>
              <TableCell sx={{ whiteSpace: 'nowrap', py: 0.5, px: 2 }}>
                {row.loan.loanAmount - (row.loan.loanAmount - row.closingCharge)}
              </TableCell>
              ...
              <TableCell sx={{ whiteSpace: 'nowrap', py: 0.5, px: 2 }}>{row.loan.loanAmount}</TableCell>
              <TableCell sx={{ whiteSpace: 'nowrap', py: 0.5, px: 2 }}>
                {row.paymentDetail.cashAmount ? row.loan.loanAmount : 0}
              </TableCell>
              <TableCell sx={{ whiteSpace: 'nowrap', py: 0.5, px: 2 }}>
                {row.paymentDetail.bankAmount ? row.loan.loanAmount : 0}
              </TableCell>
              <TableCell sx={{ whiteSpace: 'nowrap', py: 0.5, px: 2 }}>
