...
export default function SecureLoanpayhistoryTableRow({ row, selected, onDeleteRow, loanStatus }) {
  const {
    loanNo,
    customer,
    scheme,
      ...
      <TableRow hover selected={selected} sx={{ backgroundColor: statusColors[status] || '' }}>
        ...
        <TableCell sx={{ whiteSpace: 'nowrap' }}>
          {getResponsibilityValue('update_loan_pay_history', configs, user) ? (
            <Link
              to={paths.dashboard.secureloanPayHistory.edit(_id)}
        ...
        <TableCell sx={{ whiteSpace: 'nowrap' }}>
          {scheme.interestRate > 1 ? 1 : scheme.interestRate}
        </TableCell>
...
SecureLoanpayhistoryTableRow.propTypes = {
