...
export function useGetAllUnsecureSecureInterest(loanId) {
  const URL = `${import.meta.env.VITE_BASE_URL}/loans/${loanId}/unsecured-interest-payment`;
  const {
    data,
    isLoading,
    error,
    isValidating,
    mutate: refetchLoanInterest,
  } = useSWR(URL, fetcher);
